...
SRC = sys.argv[1] if len(sys.argv) > 1 else str(Path(__file__).resolve().parent.parent / "STAR.EXE.asm")
OUT = sys.argv[2] if len(sys.argv) > 2 else "strings.json"
...
while i < len(lines):
    ...
    if m:
        ...
        if abs(len(txt) - ln) <= 1 and ln > 0:
            ...
            i = j; continue
    i += 1
json.dump(strings, open(OUT,"w"), indent=0)
